...
<template>
  <header class="hidden laptop:block fixed top-0 left-0 right-0 bg-white z-50" style="box-shadow: 0 0 10px 3px grey">
    <ContentNavigation v-slot="{ navigation }">
      <nav class="container mx-auto flex justify-between px-6">
        <logo />
        <ul class="flex">
          <li v-for="link of navigation" :key="link._path"
            class="ml-2 p-4 relative flex justify-center items-center cursor-pointer" @mouseenter="mouseoverHandler"
                ...
                <template v-if="child.children">
                  ...
                  <ul class="ml-4">
                    <li v-for="c in child.children" :key="c._path">
                      <NuxtLink class="block hover:text-[#73a294] py-1" :to="`${c._path}`">{{ c.title }}</NuxtLink>
                    </li>
                  </ul>
                </template>
                <template v-else>
                  <NuxtLink class="block hover:text-[#73a294] py-1" :to="`${child._path}`">{{ child.title }}</NuxtLink>
                </template>
              </li>
            </ul>
          </li>
